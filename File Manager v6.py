...
class FileManager(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        # Title Label
        title_label = ctk.CTkLabel(top_frame, text="File Manager", font=("Helvetica", 20), anchor='center')
        title_label.pack(fill=ctk.X, padx=10, pady=10)

        self.path_label = ctk.CTkLabel(top_frame, text="Current Directory: " + self.current_path, anchor='w')
        self.path_label.pack(fill=ctk.X, padx=10, pady=5)
        ...
        self.locations_listbox = Listbox(left_frame, bg="#2b2b2b", fg="white", selectbackground="#3d85c6", highlightthickness=0, bd=0)
        ...
        self.add_to_quick_access_button = ctk.CTkButton(left_frame, text="Add to Quick Access", command=self.add_to_quick_access)
        self.add_to_quick_access_button.pack(pady=10)
        ...
        self.listbox = Listbox(right_frame, bg="#2b2b2b", fg="white", selectbackground="#3d85c6", highlightthickness=0, bd=0)
        ...
        self.zip_button = ctk.CTkButton(bottom_frame, text="Zip", command=self.zip_item)
        self.zip_button.pack(side=ctk.LEFT, padx=5, pady=5)
    ...
    def add_to_quick_access(self):
        if not self.listbox.curselection():
            messagebox.showwarning("Warning", "No folder selected to add to Quick Access")
            return
        selected_item = self.listbox.get(self.listbox.curselection())
        selected_path = os.path.join(self.current_path, selected_item)
        if os.path.isdir(selected_path):
            folder_name = os.path.basename(selected_path)
            self.common_locations[folder_name] = selected_path
            self.populate_locations()
        else:
    ...
    def zip_item(self):
        ...
        if not selected_items:
            messagebox.showwarning("Warning", "No item selected to zip")
            return

        selected_paths = [os.path.join(self.current_path, self.listbox.get(item)) for item in selected_items]
        ...
        try:
            ...
            messagebox.showinfo("Success", "Items zipped successfully")
        except Exception as e:
            messagebox.showerror("Error", f"Error zipping items: {str(e)}")
